...
source ../elasticdump-env

filename=`date +%Y%m%d`
# dump the ES data
elasticdump --input=http://<ex server>:<es port>/occikb --output=./$filename.json --type=data
...
# 10 5 * * 0 find ./ -name '*.json' -type f -mtime -1 -exec scp {} root@<backup server>:/<backup folder> \;
############################################################
